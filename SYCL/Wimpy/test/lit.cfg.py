...
backend=lit_config.params.get('sycl_be', "")
target_devices=lit_config.params.get('target_devices', 'host,cpu,gpu,acc')

lit_config.note("Backend: {BACKEND}".format(BACKEND=backend))
cuda = False
if ( backend == "PI_OPENCL" ):
    config.available_features.add('opencl')
elif ( backend == "PI_CUDA" ):
...
if 'cpu' in target_devices.split(','):
...
if 'gpu' in target_devices.split(','):
    found_at_least_one_device = True
...
if 'acc' in target_devices.split(','):
    found_at_least_one_device = True
